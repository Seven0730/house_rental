    ...
    <div>
        ...
        <div class="app-container">
            ...
            >
                ...
                <el-table-column align="center" header-align="center" label="区域">
                    <template #default="scope">
                        {{ scope.row.categoryname }}
                    </template>
                </el-table-column>
                <el-table-column align="left" header-align="center" label="房东名称">
                    <template #default="scope">
                        {{ scope.row.title }}
                    </template>
                </el-table-column>

                <el-table-column align="left" header-align="center" label="宣传口号" show-overflow-tooltip>
                    <template #default="scope">
                        {{ scope.row.slogan }}
                    </template>
                </el-table-column>

                <el-table-column align="center" header-align="center" label="上线日期" width="120">
                    <template #default="scope">
                        {{ scope.row.createdate }}
                    </template>
                </el-table-column>

                <el-table-column align="center" header-align="center" label="联系方式" width="120">
                    <template #default="scope">
                        {{ scope.row.contact }}
                    </template>
                </el-table-column>
...
onMounted(() =>
 {
            getList()
             ...
             function getList() {
                ...
                axios_request.get('/admin/landlord/getAll.action',{                params: listQuery.value            }).then(response => {
                    listData.value = response.data.landlordList
            ...
            function handleDelete(scope){
                ElMessageBox.confirm("确定要进行此操作吗?", "提示").then(() => {
                    //删除
                    const id = scope.row.landlordid
                    axios_request.post('/admin/landlord/delete.action?id='+id).then(response => {
                        ElMessage({
            ...
            function handleUpdate(scope){
                ...
                dialogFormId.value = parseInt(scope.row.landlordid) //当前操作的ID
