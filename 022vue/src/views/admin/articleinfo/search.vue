            ...
            >
                ...
                <el-table-column align="center" header-align="center" label="图片"
                    ...
                    <template #default="scope">
                        <router-link target="_blank" :to="'/articleinfoDetail?id='+scope.row.articleid">
                            <img :src="BASE_API+scope.row.image" style="width: 60px;height: 60px">

                        </router-link>
                    </template>
                </el-table-column>
                <el-table-column align="left" header-align="center" label="标题" width="400"
                ...
                <el-table-column align="center" header-align="center" label="分类">
                    <template #default="scope">
                        {{ scope.row.bannername }}
                    </template>
                </el-table-column>
                <el-table-column align="center" header-align="center" label="是否置顶"  width="100">
                    <template #default="scope">
                        {{ scope.row.istop }}
                    </template>
                </el-table-column>
                <el-table-column align="center" header-align="center"  label="是否轮播"  width="100">
                    <template #default="scope">
                        {{ scope.row.isflv }}
                    </template>
                </el-table-column>
                <el-table-column align="center" header-align="center" label="发布日期"  width="120">
                    <template #default="scope">
                        {{ $filters.parseTime(scope.row.addtime, '{m}月{d}日') }}

                    </template>
                </el-table-column>
                <el-table-column align="center" header-align="center" label="点击数"  width="80">
                    <template #default="scope">
                        {{ scope.row.hits }}
                    </template>
                </el-table-column>
...
onMounted(() =>
 {
            getList()
             ...
             function getList() {
                ...
                axios_request.get('/admin/article/getAll.action',{                params: listQuery.value            }).then(response => {
                    listData.value = response.data.articleList
            ...
            function handleDelete(scope){
                ElMessageBox.confirm("确定要进行此操作吗?", "提示").then(() => {
                    //删除
                    const id = scope.row.articleid
                    axios_request.post('/admin/article/delete.action?id='+ id).then(response => {
                        ElMessage({
            ...
            function handleUpdate(scope){
                ...
                dialogFormId.value = parseInt(scope.row.articleid) //当前操作的ID
