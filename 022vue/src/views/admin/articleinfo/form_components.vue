<template>
    <div>
        ...
        >
            ...
            <el-form-item label="分类" prop="bannerid">
                <el-select size="small" v-model="postForm.bannerid" placeholder="请选择">
                    <el-option
            ...
            <el-form-item label="是否置顶" prop="istop">
                <el-radio-group v-model="postForm.istop">
                    <el-radio label='是' key='是'>是</el-radio>
                    <el-radio label='否' key='否'>否</el-radio>
                </el-radio-group>

            </el-form-item>
            <el-form-item label="是否轮播" prop="isflv">
                <el-radio-group v-model="postForm.isflv">
                    <el-radio label='是' key='是'>是</el-radio>
                    <el-radio label='否' key='否'>否</el-radio>
                </el-radio-group>
...
const props = defineProps({
        ...
        dialogFormVisible: {
            ...
            default: false
                ...
                const  bannerOptions=ref(null)//这里写注释
    ...
    emits('close') //事件通知父组件修改el-model的显示状态
            ...
            //获取数据,并初始化
             function fetchData() {
                if (props.dataId > 0) {
                    //显示编辑数据
                    axios_request.get('/admin/article/getById.action?id='+ props.dataId).then(response => {
                            if (response.data) {
                                bannerOptions.value = response.data.bannerList

                                postForm.value = response.data.article
                            }
                        }
                    )
                } else {
                    //显示添加数据
                    axios_request.get('/admin/article/create.action' ).then(response => {
                            if (response.data) {
                                bannerOptions.value = response.data.bannerList

                                postForm.value.bannerid = response.data.bannerList[0].bannerid// 首次打开,默认选中第一个
             ...
             function saveData() {
                ...
                postFormRef.value.validate(valid => {
                    ...
                    if (valid) {
                        ...
                        if (props.dataId > 0) {
                            //保存编辑数据
                            postForm.value.articleid = props.dataId
                            postForm.value.addtime = parseTime(postForm.value.addtime, '{y}-{m}-{d}')
                            axios_request.post('/admin/article/update.action', postForm.value).then(response => {
                            ...
                            axios_request.post('/admin/article/add.action', postForm.value).then(response => {
                                ElMessage({message: response.message})
                                if (response.message) {
                                    closeDialog()//必须在AJX异步过程内
                                }
