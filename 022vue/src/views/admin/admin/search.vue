    ...
    <div>
        ...
        <div class="app-container">
            ...
            >
                ...
                <el-table-column align="center" header-align="center" label="角色">
                    <template #default="scope">
                        <span v-if="scope.row.adminid>1"> 教育厅</span>
...
<!--                        {{scope.row.adminid}}-->
                        <el-button v-if="scope.row.adminid>1"    type="danger"  size="small"  @click="handleDelete(scope)">删除
                        </el-button>
                    </template>
                </el-table-column>
            </el-table>
...
onMounted(() =>
 {
            getList()
             ...
             function getList() {
                ...
                axios_request.get('/admin/admin/getAll.action',{                params: listQuery.value            }).then(response => {
                    listData.value = response.data.adminList
            ...
            function handleDelete(scope){
                ElMessageBox.confirm("确定要进行此操作吗?", "提示").then(() => {
                    //删除
                    const id = scope.row.adminid
                    axios_request.post('/admin/admin/delete.action?id='+ id).then(response => {
                        ElMessage({
            ...
            function handleUpdate(scope){
                ...
                dialogFormId.value = parseInt(scope.row.adminid) //当前操作的ID
