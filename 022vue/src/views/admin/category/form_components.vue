    ...
    <div>
        ...
        >
            ...
            <el-form-item label="分类名称" prop="categoryname">
                <el-input placeholder="请输入" v-model="postForm.categoryname"/>
            </el-form-item>
    ...
    emits('close') //事件通知父组件修改el-model的显示状态
            ...
            //获取数据,并初始化
             function fetchData() {
                if (props.dataId > 0) {
                    //显示编辑数据
                    axios_request.get('/admin/category/getById.action?id='+props.dataId).then(response => {
                            if (response.data) {

                                postForm.value = response.data.category
                            }
                        }
                    )
                } else {
                    //显示添加数据
                    axios_request.get('/admin/category/create.action' ).then(response => {
                            if (response.data) {
             ...
             function saveData() {
                ...
                postFormRef.value.validate(valid => {
                    ...
                    if (valid) {
                        ...
                        if (props.dataId > 0) {
                            //保存编辑数据
                            postForm.value.categoryid = props.dataId
                            axios_request.post('/admin/category/update.action', postForm.value).then(response => {
                            ...
                            axios_request.post('/admin/category/add.action', postForm.value).then(response => {
                                ElMessage({message: response.message})
                                if (response.message) {
                                    closeDialog()//必须在AJX异步过程内
                                }
