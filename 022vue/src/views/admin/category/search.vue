...
onMounted(() =>
 {
            getList()
             ...
             function getList() {
                ...
                axios_request.get('/admin/category/getAll.action',{                params: listQuery.value            }).then(response => {
                    listData.value = response.data.categoryList
            ...
            function handleDelete(scope){
                ElMessageBox.confirm("确定要进行此操作吗?", "提示").then(() => {
                    //删除
                    const id = scope.row.categoryid
                    axios_request.post('/admin/category/delete.action?id='+ id).then(response => {
                        ElMessage({
            ...
            function handleUpdate(scope){
                ...
                dialogFormId.value = parseInt(scope.row.categoryid) //当前操作的ID
