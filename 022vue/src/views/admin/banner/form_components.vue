<template>
    <div>
        ...
        >
            ...
            <el-form-item label="分类名称" prop="bannername">
                <el-input placeholder="请输入" v-model="postForm.bannername"/>
            </el-form-item>
...
const props = defineProps({
  ...
  const                 rules= {
                    bannername: [{required: true, message: "请填写"}],
    ...
    emits('close') //事件通知父组件修改el-model的显示状态
            ...
            //获取数据,并初始化
             function fetchData() {
                if (props.dataId > 0) {
                    //显示编辑数据
                    axios_request.get('/admin/banner/getById.action?id='+ props.dataId).then(response => {
                            if (response.data) {

                                postForm.value = response.data.banner
                            }
                        }
                    )
                } else {
                    //显示添加数据
                    axios_request.get('/admin/banner/create.action' ).then(response => {
                            if (response.data) {
             ...
             function saveData() {
                ...
                postFormRef.value.validate(valid => {
                    ...
                    if (valid) {
                        ...
                        if (props.dataId > 0) {
                            //保存编辑数据
                            postForm.value.bannerid = props.dataId
                            axios_request.post('/admin/banner/update.action', postForm.value).then(response => {
                            ...
                            axios_request.post('/admin/banner/add.action', postForm.value).then(response => {
                                ElMessage({message: response.message})
                                if (response.message) {
                                    closeDialog()//必须在AJX异步过程内
                                }
