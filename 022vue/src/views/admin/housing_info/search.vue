            ...
            >
                ...
                <el-table-column align="center" header-align="center" label="房源名称">
                    <template #default="scope">
                        <router-link :to="'/housing_infoDetail?id='+scope.row.housing_infoid" target="_blank">

                            {{ scope.row.housing_infoname }}
                        </router-link>
                    </template>
                </el-table-column>
                <el-table-column align="center" header-align="center" label="可预约看房人数" >
                    <template #default="scope">
                        {{ scope.row.countrequire }}
                    </template>
                </el-table-column>
                ...
                <el-table-column align="center" header-align="center" label="房东名称">
                    <template #default="scope">
                        {{ scope.row.landlordtitle }}
                    </template>
                </el-table-column>


                <el-table-column align="center" header-align="center" label="审核" width="120">
                    <template #default="scope">
                        <el-tag type="success" v-if="scope.row.iscooled===0">正常</el-tag>
                        <el-tag type="warning" v-if="scope.row.iscooled===1">未审核</el-tag>
                        <el-tag type="danger" v-if="scope.row.iscooled===2">不通过</el-tag>
                    </template>
                </el-table-column>
                ...
                <el-table-column align="center" label="操作">
                    <template #default="scope">
                        ...
                        <el-button type="primary" size="small"
                                   @click="handleIscooled(scope,'0')">通过
                        </el-button>
                        <el-button type="danger" size="small"
                                   @click="handleIscooled(scope,'2')">不通过
                        </el-button>
...
function getList() {
    ...
    axios_request.get('/admin/housing_info/getAll.action', {params: listQuery.value}).then(response => {
        listData.value = response.data.housing_infoList
...
function handleDelete(scope) {
    ElMessageBox.confirm("确定要进行此操作吗?", "提示").then(() => {
        //删除
        const id = scope.row.housing_infoid
        axios_request.post('/admin/housing_info/delete.action?id=' + id).then(response => {
            ElMessage({
...
function handleStatus(scope, status_int) {
    ElMessageBox.confirm("确定要进行此操作吗?", "提示").then(() => {
        ...
        axios_request.get('/admin/housing_info/status.action?id=' + id + '&status=' + status_int).then(response => {
            ElMessage({
                message: response.message
            })
            if (response.message) {
...
function handleIscooled(scope, iscooled_int) {
    ElMessageBox.confirm("确定要进行此操作吗?", "提示").then(() => {
        ...
        axios_request.get('/admin/housing_info/iscooled.action?id=' + id + '&iscooled=' + iscooled_int).then(response => {
            ElMessage({
                message: response.message
            })
            if (response.message) {
...
function handleUpdate(scope) {
    ...
    dialogFormId.value = parseInt(scope.row.housing_infoid) //当前操作的ID
