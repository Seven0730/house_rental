            ...
            <el-card>
                ...
                <div class="app-container">
                    ...
                    >
                        ...
                        <el-table-column align="center" header-align="center" label="可预约看房人数" width="150">
                            <template #default="scope">
                                {{ scope.row.countrequire }}
                            </template>
                        </el-table-column>
                        <el-table-column align="center" header-align="center" label="发布日期" width="180">
                            <template #default="scope">
                        ...
                        <el-table-column align="center" header-align="center" label="审核" width="150">
                            <template #default="scope">
                        ...
                        <el-table-column align="center" label="操作" width="200">
                            <template #default="scope">
                                ...
                                <!--{{roleId}}-->
                                <el-button  text  size="small" @click="handleUpdate(scope)">编辑</el-button>
                                <el-button  text  size="small" @click="handleDelete(scope)">删除</el-button>
...
import axios_request from '@/utils/axios_request'
    import dialogForm from './housing_info_form_components'
            ...
            getList()
             ...
             function getList() {
                ...
                listLoading.value = true
                		 axios_request.get('/web/getList.action',{                params: listQuery.value            }).then(response => {
            ...
            function handleDelete(scope){
                ElMessageBox.confirm("确定要进行此操作吗?", "提示").then(() => {
                    ...
                    const id = scope.row.housing_infoid
                    		axios_request.post('/web/delete.action?id='+id).then(response => {
                        ElMessage({
            ...
            function handleStatus(scope, status_int){
                ElMessageBox.confirm("确定要进行此操作吗?", "提示").then(() => {
                    const id = scope.row.housing_infoid
                                    ...
                                    axios_request.get('/web/status.action?id=' + id + '&status=' + status_int).then(response => {
                        ElMessage({
                            message: response.message
                        })
                        if (response.message) {
