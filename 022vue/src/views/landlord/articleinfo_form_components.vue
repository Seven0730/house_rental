    ...
    emits('close') //事件通知父组件修改el-model的显示状态
            ...
            //获取数据,并初始化
             function fetchData() {
                if (props.dataId > 0) {
                    //显示编辑数据
                                    axios_request.get('/webarticle/getById.action?id='+ props.dataId).then(response => {
                            if (response.data) {
                    ...
                    //显示添加数据
                                axios_request.get('/webarticle/create.action' ).then(response => {
                            if (response.data) {
                    ...
                    if (valid) {
                        ...
                        if (props.dataId > 0) {
                            ...
                            postForm.value.addtime = parseTime(postForm.value.addtime, '{y}-{m}-{d}')
                                        axios_request.post('/webarticle/update.action', postForm.value).then(response => {
                            ...
                            //保存添加数据
                                        ...
                                        axios_request.post('/webarticle/add.actionn', postForm.value).then(response => {
                                ElMessage({message: response.message})
                                if (response.message) {
                                    closeDialog()//必须在AJX异步过程内
                                }
