...
import dialogForm from './contract_info_form_components'
...
function getList() {
    ...
    axios_request.get('/web/getListContract_info.action', {params: listQuery.value}).then(response => {
...
function handleDelete(scope) {
    ElMessageBox.confirm("确定要进行此操作吗?", "提示").then(() => {
        ...
        axios_request.post('/web/deleteContract_info.action?id=' + id).then(response => {
            ElMessage({
...
function handleUpdate(scope) {
    ...
    dialogFormId.value = parseInt(scope.row.contract_infoid) //当前操作的ID
