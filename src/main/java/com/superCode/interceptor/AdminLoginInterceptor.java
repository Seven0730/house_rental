...
public class AdminLoginInterceptor implements HandlerInterceptor {
    ...
    public String refreshToken(String token, HttpServletResponse response, HttpServletRequest request) {
        ...
        try {
            ...
            jwtEntityAdmin = (JwtEntityAdmin) JSONObject.toJavaObject(jsonObject, JwtEntityAdmin.class);//再转为jwtmodel

            //重新设定session
            HttpSession session = request.getSession(); //request是HttpServletRequest类型的


            Admin admin = adminService.getById(jwtEntityAdmin.getAdminid());
        ...
        if ((expire - currentTime) / 1000 < 300) {
            // 获取 token 数据
            // 重新生成 token
            token = JwtUtil.getJwtToken(jwtEntityAdmin, JwtUtil.DEFAULT_EXPIRE);
